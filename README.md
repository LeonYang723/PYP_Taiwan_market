...
3. **封鎖風險**：GitHub Actions 執行環境的出口 IP 屬於雲端機房，比一般家用網路更容易被平台的風控機制盯上而封鎖。這套系統的抓取方式前後調整過三次，每次都是蝦皮升級一次防護才跟著改：
   - 第一版用 Python `requests` 直接發 HTTP 請求 → 蝦皮在**連線層級（TLS 指紋）**判斷「不是真瀏覽器」，直接回 403，改 headers 沒用。
   - 第二版改用 Playwright 開真瀏覽器，但是在頁面 JS 裡自己組 API 網址呼叫 `fetch()` → 403 不見了，但蝦皮的 API 改回傳一個看似正常、其實是**風控系統錯誤物件**的 JSON（沒有真正的商品資料），代表蝦皮還會檢查這個 API 請求本身合不合理（例如少了前端風控 JS 算出來的 token）。
   - 第三版（目前這版）改成讓 Playwright **導航到真正的搜尋頁面**，讓蝦皮自己的前端完整跑一次，再攔截它自己發出的 API 回應，而不是我們自己另外組網址呼叫。
   
   **即使是現在這版也不保證長期不被擋**，如果蝦皮的風控還會看更細的行為模式（滑鼠移動、停留時間、搜尋間隔規律性等），未來還是可能需要再調整。**如果你發現連續好幾天都抓不到資料，很可能就是被擋了**，請看下面「當爬蟲被封鎖時怎麼辦」。爬蟲程式裡也內建了除錯 log：只要某個關鍵字第一頁就抓到 0 筆商品，就會把蝦皮實際回傳的內容印在 GitHub Actions 的執行紀錄裡，方便判斷新的失敗原因。
