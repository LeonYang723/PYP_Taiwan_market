...
```
① 人工上蝦皮搜尋「PYP」相關關鍵字，把看到的商品資訊
   填進 manual_data/PYP蝦皮市場手動蒐集表.xlsx
   （商品名稱、價格、賣家、累計已售出……，選填：規格評價統計）
        │
        ▼
...
**「商品資料」**（主要資料，每次都要填）：每一列 = 一個蝦皮商品頁面。上蝦皮搜尋 PYP 相關關鍵字，把看到的商品逐一填進去——商品名稱、賣家名稱、賣場連結、商品連結、價格、**累計已售出**（商品頁上寫的「已售出 N 件」）、評分、評分則數、庫存。都是商品頁面上直接看得到的數字，不用計算。黃色欄位是輸入欄，第 2 列（綠色）是範例，實際填寫時整列刪掉或蓋掉即可。
