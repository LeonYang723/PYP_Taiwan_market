...
③ scripts/build_reports.py
   ...
   估算每一月/每一季/每一年賣了幾件
   → docs/data/{meta,latest,monthly,quarterly,annual}.json
        │
        ▼
④ 把 data/raw/ 與 docs/data/ 底下更新的檔案 commit、push 回 repo

GitHub Pages（docs/ 資料夾，Navy 深藍主題）
   ├─ index.html    總覽頁：統計摘要、月/季/年銷量趨勢圖、賣家排行
   ├─ ranking.html  商品排行頁：依月/季/年排名的商品價格/尺寸/預估銷量
   └─ products.html 商品資料頁：目前追蹤中的商品最新快照、
                     商品月增量比較（逐月消化速度矩陣表）、規格別需求推估
...
**至少每季一次**（配合季報，最好在每季結束後、下一季開始前的一兩週內填），有餘力的話可以**每月做一次**。資料點越密集，報表的準確度越高——尤其是新商品剛開始追蹤的那一期，如果只有期末一次快照、沒有期初的基準值可以相減，數字會被標記「偏低估計」（詳見下方「銷量是怎麼算出來的」）。

**如果你想看「月增量比較」（商品資料頁的那張矩陣表，回答「這個 SKU 台灣一個月消化多少」這種問題），建議盡量固定在每個月同一天左右記錄一次快照**（例如每月 29 號前後）。原因：月報跟季報/年報用的是同一套「這期最後一次快照 − 上期最後一次快照」算法，只是週期換成自然月——如果某個月漏了沒記錄，下個月算出來的增量就會被合併成兩個月的量，逐月比較會失真；固定週期記錄，這張矩陣表的每欄數字才會是名副其實的「當月消化量」。

## 銷量是怎麼算出來的

蝦皮商品頁顯示的「已售出 N 件」，是**商品上架以來的累計銷量**，不是「這段期間賣了幾件」。這套系統每次蒐集一次快照，等於在時間軸上對這條累計曲線取樣。要估算「某一期賣了幾件」（月報/季報/年報都是同一套邏輯，只是週期長短不同）：

```
該期預估銷量 = 期末前最後一次快照的累計銷量 − 期初前最後一次快照的累計銷量
```

如果系統在期初之前還沒開始追蹤某個商品（例如系統剛上線、或商品是期中才第一次被填進表格），就沒有「期初基準值」可用，只能退而求其次用「該期第一次快照」當基準——這樣算出來的數字**一定會偏低**（漏掉了期初到我們第一次看到它之間賣掉的量）。這種情況在儀表板上會用「偏低估計」標籤標出來，提醒這是低估值、不是精確值。系統累積的追蹤時間越長、蒐集頻率越高，這種偏低估計的情況會越少。

商品資料頁的「商品月增量比較」矩陣表，就是把這套月報邏輯攤開成「橫向」比較：一列一個商品、一欄一個月份，儲存格是該商品當月的預估增量，最左邊有一欄小長條趨勢圖方便一眼看出成長/衰退。至少要累積 2 個月份的快照才會顯示這張表（只有 1 個月份時，增量的概念還不成立）。
